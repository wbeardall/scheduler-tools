...
def check_storage(handler: Union[ShellHandler, str, Dict[str, Any]], threshold: Union[int, float]=85, logger: Union[Logger, None]=None, 
               **kwargs):
...
def storage_tracker(): # pragma: no cover
    ...
    if args.service:
        command = f"{__file__} {args.host} -t {args.threshold:.1f} -i {args.interval:.1f}"
        env_vars = {
    ...
    if systemd_service():
        while True:
            check_storage(handler=args.host,threshold=args.threshold,logger=logger,**kwargs)
            time.sleep(args.interval*60*60*24)
    else:
        logger.info("Scheduler created.")
        scheduler.add_job(check_storage, 'interval', days=args.interval,
            next_run_time=datetime.datetime.now(),
            kwargs=dict(handler=args.host,threshold=args.threshold,logger=logger,**kwargs))
