...
def _check_status_remote(host, services, show_logs):
    if show_logs:
        warnings.warn(
            "show-logs is unused on remote hosts due to the requirement for elevated privileges."
        )
    handler = ShellHandler(host)
    registered = [
        el
        for el in services
        if not handler.execute(f"test -f {get_service_file(el)}").returncode
    ]
    if len(registered) == 0:
        print(f"No `schedtools` services are registered with `systemd` on host {host}.")
        return
    print(
        f"The following services are registered on host {host}: \n"
        + "\n".join(registered)
    )
    terminal_width, _ = shutil.get_terminal_size()
    print("_" * terminal_width)
...
def check_status():
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="?",
        help="Optional host on which to check status. "
