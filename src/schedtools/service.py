...
def copy_config():
    ...
    if os.path.exists(user_config):
        for file in os.listdir(user_config):
            subprocess.call(
                ["sudo", "cp", "-n", os.path.join(user_config, file), root_config]
            )
...
"""
    ...
    subprocess.call(
        ["sudo", "bash", "-c", "echo '" + service_def + "' > " + service_file]
    )
...
def remove_service(name: str):
    ...
    assert (
        name in allowed
    ), f"We do not allow removal of arbitrary services with this script. Only {allowed} can be removed, if registered."
    service_id = name.lower().replace(" ", "-")
    service_file = get_service_file(service_id)
    service_conf = f"/etc/{service_id}-service.conf"
    subprocess.call(["sudo", "systemctl", "stop", service_id])
