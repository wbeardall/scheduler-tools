...
class SMTPCredentials:
    def __init__(
        self, server, port, sender_address, password, destination_address=None
    ):
        self.server = server
        self.port = int(port)
...
def create_credentials():
    while True:
        server = (
            input(f"SMTP server (defaults to '{default_server}'): ") or default_server
        )
    ...
    destination_address = (
        input(f"Destination address (defaults to '{sender_address}'): ")
        or sender_address
    )
