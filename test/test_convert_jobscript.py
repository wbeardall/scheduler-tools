...
def test_conversion_interface(format, level, to_destroy):
    file = os.path.join(
        os.path.dirname(__file__), "dummy.pbs" if format == "slurm" else "dummy.sbatch"
    )
    new = os.path.join(
        tempfile.gettempdir(), "tmp.pbs" if format == "slurm" else "tmp.sbatch"
    )
    dst = os.path.join(
        tempfile.gettempdir(), "tmp.sbatch" if format == "slurm" else "tmp.pbs"
    )
