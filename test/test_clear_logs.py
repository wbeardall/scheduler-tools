...
def test_clear_cluster_logs(pattern, recursive, to_destroy):
    ...
    if pattern=="pbs":
        ...
        no_clear = [
            "dummylog2.pbs.o12345",
            "dummylog2.pbs.e12345",
            "notalog.md"
        ]
    else:
        to_clear = [
    ...
    for block, should_exist in [[to_clear, False], [no_clear, True]]:
        for file in block:
            assert os.path.exists(os.path.join(check_dir, file)) == should_exist, os.path.join(check_dir, file) + f" should_exist {should_exist}"